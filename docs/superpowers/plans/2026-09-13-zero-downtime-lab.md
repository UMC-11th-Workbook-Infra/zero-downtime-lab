...
초당 성공/실패 막대와 p95 지연 라인. **빠진 초를 `null` 로 채우는 것이 핵심이다** — 버킷은 요청이 있었던 초에만 만들어지므로, 그냥 이어 그리면 끊긴 구간이 공백이 아니라 직선으로 이어져 버린다.
...
Expected: PASS — 2 tests (약 15초 소요)
...
측정이 도는 동안 `docker compose restart` 를 실행하면 그래프에 공백이 생기고 순단 목록에 한 줄이 찍혀야 한다. 이것이 이 도구가 제대로 동작한다는 최종 확인이다.
...
- [ ] 측정 중 대상을 재시작하면 순단이 잡히고 그래프에 공백이 보인다
